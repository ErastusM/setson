...
public function main() {
    ...
    if addFnsRes is error {
        io:println("Error occured while trying to add new fn: ", addFnsRes.message());
    } else {
        io:println("Successfully added fn: ", addFnsRes.message);
    }
    ...
    if addFnsStream is error {
        ...
        if err is error {
            ...
            if fnsRes is error {
                io:println("Attempt to retrieve addFnsRes failed: ", fnsRes.message());
            } else {
    ...
    if showAllFnsStream is error {
        io:println("Failed to display fns: ", showAllFnsStream.message());
    } else {
        error? e = showAllFnsStream.forEach(function(ShowAllFnsRes res) {
            io:println("Fn Successfully retrieved: ", res.fn.name);
        });
    }
    ...
    if showAllWithCritStream is error {
        io:println("error occured while setting up showAllWithCrit stream: ", showAllWithCritStream.message());
    } else {
        foreach ShowAllWithCritReq showAllWithCritReq in showAllWithCritReqs {
            ...
            if err is error {
                io:println("unable to send showAllWithCriteria req: ", err.message());
            }
        }
        error? err = showAllWithCritStream->complete();
        if err is error {
            io:println("unable to send showAllWithCriteria complete message: ", err.message());
        }
        ...
        while true {
            if showAllWithCritRes is error {
                io:println("unable to send showAllWithCriteria complete message: ", showAllWithCritRes.message());
                break;
            } else {
    ...
    if delRes is error {
        ...
        io:println("Successfully deleted fn: ", delRes.message);
    }
}
